...
insert into customers_bkp(customer_id, customer_name, contact_name, address, city, postal_code, country)
values (1, 'Alfreds Futterkiste', 'Maria Anders', 'Obere Str. 57', 'Berlin', '12209', 'Germany'),
       (2, 'Ana Trujillo Emparedados y helados', 'Ana Trujillo', 'Avda. de la Constitución 2222', 'México D.F.',
        '05021', 'Mexico'),
       (3, 'Antonio Moreno Taquería', 'Antonio Moreno', 'Mataderos 2312', 'México D.F.', '05023', 'Mexico'),
...
-- verify correctness oof update operation
select customers.customer_id, customers.city, customers_bkp.customer_id, customers_bkp.city
from customers,
     customers_bkp where customers.customer_id = customers_bkp.customer_id
except
select customers.customer_id, customers.city, customers_bkp.customer_id, customers_bkp.city
from customers,
     customers_bkp  where customers.customer_id = customers_bkp.customer_id;
...
update customers
set city = (select customers_bkp.city
            from customers_bkp
            where customers.customer_id = customers_bkp.customer_id)
where customers.customer_id =
      (select customers_bkp.customer_id from customers_bkp where customers.customer_id = customers_bkp.customer_id);

select count(*), city from customers where city = 'Berlin' group by city;
select count(*), city from customers where city = 'Berlin' and country = 'Germany' group by city;
...
-- Join Products and Categories INNER JOIN with USING:
select distinct product_name, price, category_name, description
from products
...
-- select customers that don't have orders
SELECT c.customer_id, c.customer_name, c.address, c.postal_code, c.country, c.city
FROM orders o
...
-- get all products with their categories info
select p.product_id, p.product_name, p.unit, p.price, c.category_name, c.description
from products p
         inner join categories c
                    on p.category_id = c.category_id;

-- how many product items in each category
...
-- how many product items of each category do we have (by quantity in descending order)
select p.category_id, c.category_name, count(*) count
from products p
         inner join categories c on p.category_id = c.category_id
group by p.category_id, c.category_name
order by count desc;

-- what is the most frequency buy product
select *
from orders;
select *
from products;

-- add new optional int column to orders with product_id reference

-- which customer have made the most orders
select c.customer_id, c.customer_name, count(*) count
from customers c
         left join orders o on c.customer_id = o.customer_id
...
-- which customer have made the least number of orders
select c.customer_id, c.customer_name, count(*) count
from customers c
         left join orders o on c.customer_id = o.customer_id
group by c.customer_id
...
select c.customer_id, c.customer_name, c.contact_name, c.address, c.city, c.postal_code, c.country
from customers c
         left join orders o on c.customer_id = o.customer_id
where o.order_id is null;

-- do we have some junk inactive orders in system:
select *
from customers c
